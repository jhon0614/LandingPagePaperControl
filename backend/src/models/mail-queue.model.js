...
export class ModeloColaCorreo {
  ...
  async encolar(correo) {
    ...
    await this.conexiones.execute(
      "INSERT INTO cola_recuperacion (correo) VALUES (?)",
      [correo],
    );
  }
  async tomar() {
  ...
  async completar(tarea) {
    await this.conexiones.execute(
      "DELETE FROM cola_recuperacion WHERE id = ? AND reserva = ?",
      [tarea.id, tarea.reserva],
    );
  }
  async limpiar() {
    await this.conexiones.execute(
      "DELETE FROM cola_recuperacion WHERE disponible_en < DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY) LIMIT 1000",
    );
    await this.conexiones.execute(
      "DELETE FROM limites_solicitudes WHERE reinicia_en < DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY) LIMIT 1000",
    );
  }
}
