...
export function centavos(valor) {
  ...
  return (
    (BigInt(entero) * 100n + BigInt(decimal.padEnd(2, "0"))) *
    (negativo ? -1n : 1n)
  );
}
...
export function importeSql(valor) {
  if (valor > MAX_CENTAVOS || valor < -MAX_CENTAVOS)
    throw new ErrorAplicacion(
      "El importe calculado excede el máximo permitido.",
      400,
...
export function importeNumero(valor) {
  if (
    valor > BigInt(Number.MAX_SAFE_INTEGER) ||
    valor < -BigInt(Number.MAX_SAFE_INTEGER)
  )
    throw new ErrorAplicacion(
      "El agregado excede la precisión admitida por la respuesta.",
      400,
      "IMPORTE_FUERA_DE_RANGO",
    );
...
export const dinero = z
  ...
  .max(9999999999.99)
  .refine(
    (valor) => /^\d+(\.\d{1,2})?$/.test(String(valor)),
