...
export async function transaccionAdministrativa(conexiones, responsableId, operacion) {
  ...
  try {
    ...
    const [responsables] = await conexion.execute(
      `SELECT u.id FROM usuarios u JOIN roles r ON r.id = u.rol_id
        ...
        AND r.nombre IN ('ADMINISTRADOR', 'DUENO') FOR UPDATE`, [responsableId],
    );
    if (!responsables[0]) throw new ErrorAplicacion("El responsable ya no tiene permiso.", 403, "ACCESO_DENEGADO");
