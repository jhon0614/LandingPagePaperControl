...
export class BaseDatos {
  ...
  constructor(configuracion) {
    if (BaseDatos.#instancia) {
      // si ya existe una instancia, no se crea otra
      throw new Error(
        "Use BaseDatos.obtenerInstancia() para obtener el grupo Singleton.",
      );
    }
      ...
      ...(configuracion.tls
        ? {
            ssl: {
              ...
              ...(configuracion.certificadoCa
                ? { ca: readFileSync(configuracion.certificadoCa, "utf8") }
                : {}),
            },
          }
        : {}),
  ...
  static obtenerInstancia(configuracion) {
    ...
    if (!BaseDatos.#instancia) {
      if (!configuracion) {
        throw new Error(
          "La configuración es obligatoria al inicializar la conexión.",
        );
      }
  ...
  async comprobarEsquema() {
    try {
      await this.conexiones.query(
        "SELECT clave FROM limites_solicitudes LIMIT 0",
      );
      await this.conexiones.query("SELECT id FROM cola_recuperacion LIMIT 0");
      await this.conexiones.query(
        "SELECT clave FROM solicitudes_venta LIMIT 0",
      );
    } catch (error) {
      if (error.code === "ER_NO_SUCH_TABLE")
        throw new Error(
          "Falta aplicar database/migrations/2026-09-09-seguridad-backend.sql antes de iniciar la API.",
        );
      throw error;
    }
  }
