...
export class AlmacenLimitesMySQL {
  ...
  clave(key) { return createHash("sha256").update(`${this.prefix}:${key}`).digest("hex"); }
  async increment(key) {
    ...
    try {
      ...
      const [filas] = await conexion.execute(
        `SELECT total, DATE_FORMAT(reinicia_en, '%Y-%m-%dT%H:%i:%s.%fZ') AS reinicia_en
          FROM limites_solicitudes WHERE clave = ?`, [clave],
      );
      await conexion.commit();
      return { totalHits: Number(filas[0].total), resetTime: new Date(filas[0].reinicia_en) };
    } catch (error) { await conexion.rollback(); throw error; }
    finally { conexion.release(); }
  }
  async decrement(key) {
    await this.conexiones.execute("UPDATE limites_solicitudes SET total = GREATEST(0, total - 1) WHERE clave = ?", [this.clave(key)]);
  }
  async resetKey(key) {
    await this.conexiones.execute("DELETE FROM limites_solicitudes WHERE clave = ?", [this.clave(key)]);
  }
}
